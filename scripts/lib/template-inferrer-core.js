 ...
 * @param {string} localeXml - CSL locale XML content
 * @returns {Object|null} { entries: Array<string>, style } or null if rendering fails
 */
function renderWithCiteproc(styleXml, testItems, localeXml) {
  try {
...
function aggregateByType(entries, refByEntry) {
  ...
  for (let idx = 0; idx < entries.length; idx++) {
    ...
    typedComponents[type].entries.push({ index: idx, rendered: entry, refData });
...
function findConsensusOrdering(entries, refByEntry) {
  ...
  for (const [type, data] of Object.entries(typedComponents)) {
    const typeOrderings = [];
    for (const { rendered, refData } of data.entries) {
      const ordering = analyzeOrdering(rendered, refData);
  ...
  for (const comp of allComponents) {
    componentScores[comp] = 0;
    for (const other of allComponents) {
      if (other === comp) continue;
      componentScores[comp] +=
        (precedence[comp]?.[other] || 0) - (precedence[other]?.[comp] || 0);
    }
  }

  const componentList = [...allComponents];
  componentList.sort((a, b) => {
    const scoreDelta = (componentScores[b] || 0) - (componentScores[a] || 0);
    if (scoreDelta !== 0) {
      return scoreDelta;
    }
    const frequencyDelta = (componentFrequency[b] || 0) - (componentFrequency[a] || 0);
    if (frequencyDelta !== 0) {
...
function inferTemplateFromInputs({
  styleXml,
  section = 'bibliography',
  testItems,
  localeXml,
}) {
  if (!styleXml || !testItems || !localeXml) return null;

  let rendered;
  if (section === 'bibliography') {
    rendered = renderWithCiteproc(styleXml, testItems, localeXml);
  } else {
    const citations = renderCitations(styleXml, testItems, localeXml);
  ...
  const { consensusOrdering, typedComponents, componentFrequency, orderingAgreementByType } = findConsensusOrdering(
    rendered.entries,
    refByEntry,
  );
  ...
  if (section === 'citation') {
    const integral = inferTemplateFromInputs({
      styleXml,
      section: 'citation-integral',
      testItems,
      localeXml,
    });
    const non_integral = inferTemplateFromInputs({
      styleXml,
      section: 'citation-non-integral',
      testItems,
      localeXml,
    });
...
function buildFragmentOutput(result, section, styleName = 'unknown') {
...
function inferTemplateFragment(inputs) {
  const result = inferTemplateFromInputs(inputs);
  return buildFragmentOutput(result, inputs.section || 'bibliography', inputs.styleName);
}

module.exports = {
  buildFragmentOutput,
  inferTemplateFromInputs,
