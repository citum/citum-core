...
if ! search_q '^pub use citum_schema_style::\*;$' "$lib_rs"; then
  echo "error: $lib_rs must re-export citum_schema_style" >&2
  exit 1
fi

if ! search_q '^pub mod data \{$' "$lib_rs"; then
  echo "error: $lib_rs must expose the data facade module" >&2
  exit 1
fi

if ! search_q '^    pub use citum_schema_data::\*;$' "$lib_rs"; then
...
done < <(search_n '^[[:space:]]*(pub[[:space:]]+)?mod[[:space:]]+' "$lib_rs" || true)
