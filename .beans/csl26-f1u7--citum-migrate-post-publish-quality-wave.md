...
| PR4 | [[csl26-ly8d]] | todo | Extend `--minimize-wrapper` to template-link and independent-parent-link candidates (mdpi, numeric cluster). |
| PR4b | [[csl26-tjqn]] | todo | Make minimization the **default** outcome when oracle evidence proves equivalence. Today the user must pass `--family-candidate auto --minimize-wrapper` to get the small form; default invocation still emits the standalone. |
