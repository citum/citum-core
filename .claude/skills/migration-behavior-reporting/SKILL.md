...
description: >
  Expand or maintain the migration behavior coverage report in citum-core. Use
  this skill when converting reviewer-facing citum-migrate suites into
...
## Use This Skill When
- You are converting another `crates/citum-migrate/tests/*.rs` suite into behavior-oriented scenarios.
...
- You are changing low-level unit tests under `crates/citum-migrate/src/` that are not suitable for reviewer-facing behavior coverage.
- You are trying to make the full `citum-migrate` test inventory into the human report.
- You are expanding engine reporting; use `engine-behavior-reporting` for that.
...
## Expansion Workflow
1. Pick the next migration suite with clear user-visible behavior.
   Prefer acceptance-style or reviewer-meaningful migration outcomes over internal transform checks.
2. Rewrite that suite to match the report shape:
...
3. Ensure the scenario text describes observable migration outcomes, not implementation trivia.
...
4. Add the suite to the migration reporting pipeline:
   - add its binary-to-source mapping in `scripts/generate-test-report.py` via the wrapper script
   - add it to `scripts/migration-test-report.sh` only if it belongs in the default published migration report
5. Regenerate the report with `./scripts/migration-test-report.sh`.
6. Read `target/migration-behavior-report.md` or `target/migration-behavior-report.html` and confirm it is useful to a human reviewer without opening source.

## Source Reference Rules
- The report should emit concise repo-relative source references like `crates/citum-migrate/tests/date_inference.rs:63-68`.
...
- Keep migration-report publication in `main`-only or manual docs workflows unless the user explicitly asks for PR-time publication.
