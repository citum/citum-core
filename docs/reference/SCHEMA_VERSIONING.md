...
| **Schema** | Citum schema format exposed by `citum_schema::SCHEMA_VERSION` | `STYLE_SCHEMA_VERSION` in `crates/citum-schema-style/src/lib.rs` | automated release workflow + `Schema-Bump:` footer |
...
Code releases are prepared by `.github/workflows/release.yml`.

1. When a `feat:` or breaking commit merges to `main`, the workflow detects path changes.
2. It runs `cargo release <level> --workspace` to bump versions and generate the changelog.
3. It opens or updates a release PR on the `release/next` branch.
4. When the release PR is merged, the workflow auto-creates the `v*` tag.
