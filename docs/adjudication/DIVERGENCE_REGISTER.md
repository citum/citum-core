...
| div-006 | `new-harts-rules-notes`, `new-harts-rules-notes-label-page`, `new-harts-rules-notes-no-url` | Patent bibliography author name-order: first author inverted (family-first), subsequent authors given-first | All authors inverted when `name-order: family-first` is used in contributor config | Citum `NameOrder` enum only supports global inversion; per-author or positional inversion not yet supported | Citum design principle | Patent entries render all names inverted (e.g. `Johnson, David, and Lee, Sarah`) instead of mixing (e.g. `Johnson, David, and Sarah Lee`); classified as acceptable Phase 1 gap, documented here to prevent regression |
